...
def edit_template(service: str, account: str, new_command: str,
                  secrets: dict[str, str] | None = None) -> None:
    """修改已 seal runner 的命令模板（in-place）。

    语义（Round 3 Q1=B / Q2=Q3=Q4=A）：
      - label / output_path / secret_names 保持不变（从旧 comment 读）
      - K 同时轮换（新 binary → 新 cdhash → 新 ACL → 新 K）
      - 新模板的 ``{{secret:NAME}}`` 集合必须与旧一致
      - legacy runner（无 comment）会被 cmdseal.py 拒绝
      - 零额外钥匙串授权弹窗（不读旧 K）

    AEAD 密文无法回解旧 secret 值，若模板中含 secret 占位符，
    ``secrets`` 必须提供完整值（由 GUI 向用户重新收集）。
    失败时抛 CalledProcessError；UI 层捕获展示 stderr。
    """
    if not CMDSEAL_PY.is_file():
        raise FileNotFoundError(f"cmdseal.py not found at {CMDSEAL_PY}")
    argv = [
        PYTHON_EXE, str(CMDSEAL_PY), "edit-template",
        "--service", service,
        "--user", account,
        "--command", new_command,
        "--secrets-from-stdin",
    ]
    input_text = "".join(f"{k}={v}\n" for k, v in (secrets or {}).items())
    res = subprocess.run(
        argv,
        input=input_text,
        text=True,
